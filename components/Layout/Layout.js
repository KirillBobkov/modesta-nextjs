...
const getMetaConfig = (pathname) => {
  ...
  if (pathname.includes("buy")) {
    return {
      ...
      description: "На этой странице вы найдете всю необходимую информацию о том, как получить протез, какие документы для этого нужны и какой порядок действий необходимо предпринять. Мы поможем вам на каждом этапе этого процесса. Не откладывайте свою мечту - начните менять свою жизнь уже сегодня!”",
      keywords:
  ...
  if (pathname.includes("roadmap")) {
    return {
      ...
      description:
        "Мы гордимся своей историей и тем, как мы прошли этот путь. С самых первых дней нашей работы мы стремились помочь людям вернуться к полноценной жизни. Мы создавали протезы, основываясь на новейших технологиях и исследованиях. И сегодня мы с уверенностью можем сказать, что наши усилия были не напрасны. Мы не останавливаемся на достигнутом и продолжаем развиваться, чтобы предлагать вам еще более качественные и удобные протезы. Наша история продолжается, и мы приглашаем вас стать ее частью!",
      keywords:
