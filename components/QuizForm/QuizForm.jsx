...
export function QuizForm() {
  ...
  function handleSend() {
    ...
    const sendMessage = () => {
      ...
      fetch(url, {
        ...
        .then((response) => {
          setResponseMessage(
            "Спасибо, ваши ответы отправлены. Спаси Мы свяжемся с вами в ближайшее время."
          );
  ...
  return (
    <VisibilityManager className={styles['quiz-block']} style={{ backgroundImage: `url(${formBack.src})` }} id="submit">
      <VisibilityManager
        as="div"
        onInit={false}
        className={`${styles['quiz-form']} ${shadowStyles.shadow}`}
      >
